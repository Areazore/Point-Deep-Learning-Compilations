...
# 这个文件实现了PointNet++用于语义分割的单尺度分组(SSG)模型
# 语义分割任务是为点云中的每个点分配一个语义类别标签
#
# 四个模型的区别：
# 1. 任务类型区别：
...
class get_model(nn.Module):
    def __init__(self, num_classes):
        ...
        self.sa1 = PointNetSetAbstraction(1024, 0.1, 32, 9 + 3, [32, 32, 64], False)  # 第一层SA: 1024个中心点, 半径0.1
        self.sa2 = PointNetSetAbstraction(256, 0.2, 32, 64 + 3, [64, 64, 128], False)  # 第二层SA: 256个中心点, 半径0.2
        self.sa3 = PointNetSetAbstraction(64, 0.4, 32, 128 + 3, [128, 128, 256], False)  # 第三层SA: 64个中心点, 半径0.4
        self.sa4 = PointNetSetAbstraction(16, 0.8, 32, 256 + 3, [256, 256, 512], False)  # 第四层SA: 16个中心点, 半径0.8
        
        # 特征传播层 (Feature Propagation layers) - 上采样并传播特征
        # 参数为: in_channel(输入特征维度), mlp(多层感知机通道数)
        self.fp4 = PointNetFeaturePropagation(768, [256, 256])  # 第四层FP: 从16点传播到64点
        self.fp3 = PointNetFeaturePropagation(384, [256, 256])  # 第三层FP: 从64点传播到256点
        self.fp2 = PointNetFeaturePropagation(320, [256, 128])  # 第二层FP: 从256点传播到1024点
    ...
    def forward(self, xyz):
        ...
        # 设置抽象(SA)层: 逐步下采样并提取特征
        l1_xyz, l1_points = self.sa1(l0_xyz, l0_points)  # 1024个点
        l2_xyz, l2_points = self.sa2(l1_xyz, l1_points)  # 256个点
        l3_xyz, l3_points = self.sa3(l2_xyz, l2_points)  # 64个点
        l4_xyz, l4_points = self.sa4(l3_xyz, l3_points)  # 16个点
